...
1.  Given the corpus of common words in `stringr::words`, create regular
    ...
    `str_extract()` to show only the matching or non-matching words.
...
str_extract_all("abababa", "aba")
```

Note the use of `str_extract_all()`. As you'll shortly learn, many stringr functions come in pairs: one function works with a single match, and the other works with all matches. The second function will have the suffix `_all`.
